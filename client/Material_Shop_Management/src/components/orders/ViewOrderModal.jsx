...
import { fetchInvoice, clearInvoice } from "../../redux/reportSlice";
import InvoiceSlip from "../reports/InvoiceSlip";
...
function ViewOrderModal({ order, allCustomerOrders, onClose }) {
  ...
  const handleShowInvoice = () => {
    dispatch(fetchInvoice(order._id));
    setShowInvoice(true);
  };
        ...
        <div className="relative bg-white rounded-2xl shadow-lg w-full max-w-lg p-6 z-10 max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex gap-2">
            <button onClick={handleShowInvoice}
              className="flex-1 h-10 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition flex items-center justify-center gap-2">
              <svg className="w-4 h-4" viewBox="0 0 16 16" fill="none">
              ...
              {allCustomerOrders && allCustomerOrders.length > 1 ? "Combined Invoice" : "Invoice"}
            </button>
            <button onClick={onClose}
