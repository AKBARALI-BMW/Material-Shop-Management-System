...
function Dashboard() {
  ...
  // ── Step 4 — Due Date Overdue Checker ─────────────────────────
  useEffect(() => {
    ...
    orders.forEach((order) => {
      if (!order.dueDate || !order.dueAmount || order.dueAmount <= 0) return;
      ...
      if (dueDate < today) {
        ...
        const exists = notifs.some(
          (n) => n.type === "overdue" && n.orderId === order._id
        );
        ...
        if (!exists) {
          dispatch(addNotification({
            ...
            message:      `${order.customer?.name || "Customer"} — Rs ${(order.dueAmount || 0).toLocaleString()} overdue since ${new Date(order.dueDate).toLocaleDateString()}`,
            orderId:      order._id,
    ...
    <Layout>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {loading ? Array(4).fill(0).map((_, i) => (
          ...
          { label: "Total Revenue",   value: `Rs ${stats.totalRevenue.toLocaleString()}`,   color: "text-slate-800", bg: "bg-indigo-50",
            icon: <svg className="w-5 h-5 text-indigo-600" viewBox="0 0 16 16" fill="none"><path d="M8 1v14M5 4h4.5a2.5 2.5 0 010 5H5m0 0h5a2.5 2.5 0 010 5H5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg> },
          { label: "Total Orders",    value: stats.totalOrders,                             color: "text-slate-800", bg: "bg-blue-50",
            icon: <svg className="w-5 h-5 text-blue-600" viewBox="0 0 16 16" fill="none"><path d="M3 2h10l1 4H2L3 2z" stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round"/><path d="M2 6v7a1 1 0 001 1h10a1 1 0 001-1V6" stroke="currentColor" strokeWidth="1.3"/><path d="M6 10h4" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg> },
          { label: "Total Customers", value: stats.totalCustomers,                          color: "text-slate-800", bg: "bg-green-50",
            icon: <svg className="w-5 h-5 text-green-600" viewBox="0 0 16 16" fill="none"><circle cx="6" cy="5" r="3" stroke="currentColor" strokeWidth="1.3"/><path d="M1 14c0-2.76 2.24-5 5-5s5 2.24 5 5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/><path d="M12 7c1.1 0 2 .9 2 2s-.9 2-2 2M14 14c0-1.1-.9-2-2-2" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg> },
          { label: "Total Due",       value: `Rs ${stats.totalDue.toLocaleString()}`,       color: "text-red-600",   bg: "bg-red-50",
            icon: <svg className="w-5 h-5 text-red-500" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="7" stroke="currentColor" strokeWidth="1.3"/><path d="M8 4v4l3 2" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg> },
        ].map((s) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-5">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-5">
          ...
          {loading ? <Skeleton className="h-52 w-full" /> : <div className="h-52"><BarChart data={monthlyRevenue} /></div>}
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          ...
          {loading ? <Skeleton className="h-52 w-full" /> : <div className="h-52"><DonutChart data={orderStatus} /></div>}
        </div>
      </div>
              ...
              <table className="w-full text-sm min-w-[480px]">
                ...
                <tbody>
                  {recentOrders.length === 0 ? (
                    <tr><td colSpan={5} className="py-8 text-center text-slate-400 text-sm">No orders yet.</td></tr>
                  ) : recentOrders.map((o) => (
                    <tr key={o._id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition">
                      <td className="py-2.5 pr-4">
                        <span className="text-xs font-mono font-semibold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-md">{o.orderNumber}</span>
                      </td>
                      <td className="py-2.5 pr-4 text-slate-800 text-xs font-medium whitespace-nowrap">{o.customer?.name || "—"}</td>
                      <td className="py-2.5 pr-4 text-slate-800 text-xs font-medium whitespace-nowrap">Rs {(o.totalAmount || 0).toLocaleString()}</td>
              ...
              {lowStock.map((item) => (
                <div key={item._id} className="flex items-center justify-between p-2.5 bg-slate-50 rounded-lg border border-slate-100">
                  ...
                  <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium flex-shrink-0 ml-2 ${
                    item.status === "Out Stock" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"
                  }`}>
                    {item.status}
                  </span>
                </div>
            ...
            <table className="w-full text-sm min-w-[500px]">
              ...
              <tbody>
                {topCustomers.length === 0 ? (
                  <tr><td colSpan={6} className="py-8 text-center text-slate-400 text-sm">No customers yet.</td></tr>
                  ...
                  return (
                    <tr key={c._id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition">
                      <td className="py-3 pr-4">
                        <span className={`text-xs font-bold ${index === 0 ? "text-amber-500" : index === 1 ? "text-slate-400" : index === 2 ? "text-orange-400" : "text-slate-300"}`}>
                          #{index + 1}
                        </span>
                      </td>
                      <td className="py-3 pr-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-slate-800 font-medium whitespace-nowrap text-xs">{c.name}</span>
                        </div>
                      </td>
                      <td className="py-3 pr-4 text-slate-800 text-xs">{c.totalOrders || 0}</td>
                      <td className="py-3 pr-4 text-slate-800 font-medium text-xs whitespace-nowrap">Rs {(c.totalAmount || 0).toLocaleString()}</td>
                      <td className="py-3 pr-4 text-xs whitespace-nowrap">
                        <span className={due > 0 ? "text-red-600 font-medium" : "text-slate-400"}>Rs {due.toLocaleString()}</span>
                      </td>
                      <td className="py-3">
